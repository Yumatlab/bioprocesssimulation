function app_new = Bacillus_academii(app)
%% Open-loop feed control
% % 

% Only one reservoir avaliable
app.FR1(end+1) = 0;
...
%% pO2 control
            ...
            if app.VL(end) > 0
                ...
                if app.switch_exp || app.switch_pulse
                    n = num2str(app.currentReservoir);
                    if app.(['f_FR' n]) == 1
                        DR1 = app.(['FR' n])(end)/app.VL(end); % Dilution rate [1/h]
                   end
                end
...
%% New method
% Define ODE function for volume calculation
            ODE_Vol       = @(t,yV) Bacillus_ODE_Volume(app.FR1(end),app.FH(end),app.FT1,app.FT2,t,yV);

            % Liquid volume balance
            [~,yV] = ode45(ODE_Vol,[0 app.deltat],[app.VL(end) app.VR1(end) app.VT2(end) app.VT1(end)]);
            app.VL(end+1)  = yV(end,1);
            %app.mLkgEditField.Value = app.VL(end)*app.rhoL;
            ...
            ODE_concbal = @(t,y) Bacillus_ODE_Luttmann(app.qXpX(end),Din,app.cS1R1,qS1pX,app.cS2R1,qS2pX,app.CPR1tot,qPpX,app.MP,app.cOT1,app.cOT2,app.cOR1,app.OTR(end),app.OUR(end),app.cOL100,app.TMpO2,DT1,app.CAcT1tot,qAcpX,app.MAc,DT2,app.CAlT2tot,qAlpX,AlTR,app.MAl,DR1,app.CCR1tot,app.CCT1tot,app.CCT2tot,app.CTR(end),CER,app.MCO2,app.pHL(end),app.TMpH,app.TMxO2,app.xOG(end),app.TMxCO2,app.xCG(end),lambdaF,app.qhpX,lambdaAF,AAFin,app.tauD,app.DD,app.thetaTc,app.tauDL,app.thetaU,app.tauDU,tauL,tauLD,tauLU,QdotM,QdotSt,CL,t,y);
