function app_new = Escherichia_coli(app)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    elseif app.switch_pulse
        %% Pulse feed phase
        app.v.FR1(idx) = app.p.FR1max * app.p.kR1;
    ...
    elseif app.p.Mode_pO2 == 4
        ...
        app.a.cP_feedpO2 = app.a.ce_feedpO2(idx)*app.p.KP_feedpO2; % P-part of controller with KP = -2.0
        app.a.cI_feedpO2 = app.a.cI_feedpO2+(app.a.ce_feedpO2(idx)+app.a.ce_feedpO2(previdx))/2*app.a.deltat*app.p.KI_feedpO2; % I-part of controller with KI = -15.0 and deltat
        app.a.cD_feedpO2 = (app.a.ce_feedpO2(idx)-app.a.ce_feedpO2(previdx))/app.a.deltat*app.p.KD_feedpO2; % D-part of controller with KD = -0.009
    ...
    elseif app.p.f_feed == 1 % feed control as second priority
        ...
        elseif app.p.Mode_feed == 1
            ...
            cSLw = app.p.(['cS' n 'Lw']);
            cSL = app.v.(['cS' n 'L'])(previdx); % Here the measured value as a controlled value
            ...
            app.a.(['cI_feedR' n])        = app.a.(['cI_feedR' n]) + (app.a.(['ce_feedR' n])(idx) + app.a.(['ce_feedR' n])(previdx)) / 2 * app.a.deltat * app.p.(['KI_feedR' n]); % I-part of controller with KI = 1000 and with time increment deltat
            app.a.(['cD_feedR' n])        = (app.a.(['ce_feedR' n])(idx)-app.a.(['ce_feedR' n])(previdx))/app.a.deltat*app.p.(['KD_feedR' n]); % D-part of controller with KD = 0.015
    ...
    switch app.p.Mode_pO2
        case 1 % pO2 agitation
            ...
            % Calculate controller gains
            app.a.cP_agi        = app.a.ce_agi(idx)*app.p.KP_agi; % P-part of controller with KP = 10.0
            app.a.cI_agi        = app.a.cI_agi + (app.a.ce_agi(idx) + app.a.ce_agi(previdx)) / 2 * app.a.deltat * app.p.KI_agi; % I-part of controller with KI = 1000 and with time increment deltat
            app.a.cD_agi        = (app.a.ce_agi(idx)-app.a.ce_agi(previdx))/app.a.deltat*app.p.KD_agi; % D-part of controller with KD = 0.015 and with time increment deltat PARAMETER ANGEPASST VON 0.25, DA SONST SCHWINGUNG ZU GROß
        ...
        case 2 % pO2 aeration
            ...
            app.a.ce_aeration(idx) = app.a.cE_aeration/(100-1); % Normalized controller difference e = (w-x)/(w_max - w_min); [-1,+1]

            % Calculate controller gains
            app.a.cP_aeration = app.a.ce_aeration(idx)*app.p.KP_aeration; % P-part of controller with KP = 20.0
            app.a.cI_aeration = app.a.cI_aeration+(app.a.ce_aeration(idx)+app.a.ce_aeration(previdx))/2*app.a.deltat*app.p.KI_aeration; % I-part of controller with KI = 0.003 with time increment 0.005 h
            app.a.cD_aeration = (app.a.ce_aeration(idx)-app.a.ce_aeration(previdx))/app.a.deltat*app.p.KD_aeration; % D-part of controller with KD = 0.0188
            ...
            app.v.FnG(idx) = app.v.FnAIR(idx)+app.v.FnO2(idx)+app.v.FnN2(idx)+app.v.FnCO2(idx);
        ...
        case 3 % pO2 gasmix
            ...
            app.a.ce_gasmix(idx) = app.a.cE_gasmix/(1-app.p.xOAIR); % Normalized controller difference e = (w-x)/(w_max - w_min); [-1,+1]

            app.a.cP_gasmix        = app.a.ce_gasmix(idx)*app.p.KP_gasmix; % P-part of controller with KP = 0.4
            app.a.cI_gasmix(idx) = app.a.cI_gasmix(previdx)+(app.a.ce_gasmix(idx)+app.a.ce_gasmix(previdx))/2*app.a.deltat*app.p.KI_gasmix; % I-part of controller with KI = 0.003 with time increment 0.005 h
            app.a.cD_gasmix        = (app.a.ce_gasmix(idx)-app.a.ce_gasmix(previdx))/app.a.deltat*app.p.KD_gasmix; % D-part of controller with KD = 0.0005

            % Calculation of new relative setpoint for xOGin
            ygasmix  = (app.a.cP_gasmix+app.a.cI_gasmix(idx)+app.a.cD_gasmix(idx))*100; % PID sum in percent
    ...
    if app.p.Mode_harvest  == 1
        ...
        % Calculate controller gains
        cP_LW            = app.a.ce_LW(idx)*app.p.KP_LW; % P-part of controller with KP = 10.0
        app.a.cI_LW(idx) = app.a.cI_LW(previdx)+(app.a.ce_LW(idx)+app.a.ce_LW(previdx))/2*app.a.deltat*app.p.KI_LW; % I-part of controller with KI = 0.3 and with time increment deltat
        cD_LW            = (app.a.ce_LW(idx)-app.a.ce_LW(previdx))/app.a.deltat*app.p.KD_LW; % D-part of controller with KD = 1.0

        % Calculation of new relative setpoint for harvest pump
        yLW  = (cP_LW+app.a.cI_LW(idx)+cD_LW)*100; % PID sum in percent
    ...
    if app.v.FnG(previdx) > 0
        app.v.xOGin(idx) = (app.p.xOAIR*app.v.FnAIR(idx)+app.v.FnO2(idx))/app.v.FnG(idx);
    ...
    elseif app.p.Mode_temp == 1
        ...
        app.a.cP_Part        = app.a.ce(idx)*app.p.KP_temp1; % P-part of controller with KP = 0.1
        app.a.cI_Part(idx) = app.a.cI_Part(previdx)+(app.a.ce(idx)+app.a.ce(previdx))/2*app.a.deltat*app.p.KI_temp1; % I-part of controller with KI = 0.01 with time increment deltat

        % Calculation of steam mass flux and cooling flux in case of steam heating at split-range
        wDJ  = app.a.cP_Part+app.a.cI_Part(idx)+app.p.thetaDJ_WP; % PI sum + Workingpoint (ThetaDJw = ThetaLw)
    ...
    ODE_Vol       = @(t,yV) Escherichia_ODE_Volume(app.v.FR1(previdx),app.v.FH(previdx),app.v.FT1(previdx),app.v.FT2(previdx),t,yV);

    % Liquid volume balance
    [~,yV] = ode15s(ODE_Vol,[0 app.a.deltat],[app.v.VL(previdx) app.v.VR1(previdx) app.v.VT2(previdx) app.v.VT1(previdx)]);
    app.v.VL(idx)  = yV(end,1);
    app.v.VR1(idx)  = yV(end,2);
    app.v.VT2(idx) = yV(end,3);
    app.v.VT1(idx) = yV(end,4);

    app.v.VR1in(idx) = app.p.VR10-app.v.VR1(previdx); % Volume added from glucose reservoir
    ...
    % Maximum specific growth rate S1 [1/h]
    my1max = app.p.my1opt*fpH*ftheta;

    % Maximum specific growth rate S2 [1/h]
    my2max = app.p.my2opt*fpH*ftheta;

    % Maximum specific growth rate S3 [1/h]
    my3max = app.p.my3opt*fpH*ftheta;
    ...
    app.a.qS2pXmax = (my2max+app.a.mySm)/app.p.yXpS2gr;

    % Maximum specific acetate uptake rate [1/h]
    app.a.qS3pXmax = (my3max+app.a.mySm)/app.p.yXpS3gr;
    ...
    if app.v.cP1L(previdx) <= 0
        app.v.cS3L(idx) = 0;
    else
        app.v.cS3L(idx) = app.v.cP1L(previdx);
    end

    % Optimum specific substrate uptake rate 3 [1/h]
    if app.v.cS3L(previdx) <= 0
        qS3pXopt = 0;
    else
        qS3pXopt = app.a.qS3pXmax*app.v.cS3L(previdx)/(app.v.cS3L(previdx)+app.p.kS3)*app.p.kI31/(app.v.cS1L(previdx)+app.p.kI31)*app.p.kI32/(app.v.cS2L(previdx)+app.p.kI32);
    end
    ...
    if qXpXSgr < qXpXOgr
        ...
        if any(app.p.f_Inoc == 1)
            %   app.a.LimitationTypeLabel.Text = "Substrate Limitation";
        end
    else
        qXpXgr = qXpXOgr;
        if any(app.p.f_Inoc == 1)
            %   app.a.LimitationTypeLabel.Text = "Oxygen Limitation";
        end
    end
    ...
    if any(app.p.f_Inoc == 1)
        app.v.qXpX(idx) = qXpXgr-app.a.mySm;
    else
        app.v.qXpX(idx) = 0;
    end
    ...
    SSGl = (qXpXgr-app.p.yXpS1gr*qS1pX)/app.p.yXpS2gr;
    if SSGl < qS2pXopt
        qS2pX = SSGl;
    else
        qS2pX = qS2pXopt;
    ...
    SSA = (qXpXgr-app.p.yXpS1gr*qS1pX-app.p.yXpS2gr*qS2pX)/app.p.yXpS3gr;
    if SSA < qS3pXopt
        qS3pX = SSA;
    else
        qS3pX = qS3pXopt;
    ...
    qP1pX = (app.p.yP1pS1*qS1pXopt+app.p.yP1pS2*qS2pXopt)*app.p.kIP1O/(app.p.kIP1O+app.v.cOL(previdx))-app.p.yP1pS3*qS3pX;
    ...
    ODE_concbal = @(t,y) Escherichia_ODE_Luttmann(app.v.qXpX(previdx),Din,app.p.cS1R1,qS1pX,app.p.cS2R1,qS2pX,app.p.cP1R1,qP1pX,app.p.MP1,app.p.cOT1,app.p.cOT2,app.p.cOR1,app.v.OTR(previdx),app.v.OUR(previdx),app.a.cOL100,app.p.TMpO2,DT1,app.p.CAcT1tot,qAcpX,app.p.MAc,DT2,app.p.CAlT2tot,qAlpX,AlTR,app.p.MAl,DR1,app.p.CCR1tot,app.p.CCT1tot,app.p.CCT2tot,app.v.CTR(previdx),CER,app.p.MCO2,app.v.pHL(previdx),app.p.TMpH,app.p.TMxO2,app.v.xOG(previdx),app.p.TMxCO2,app.v.xCG(previdx),lambdaF,app.p.qhpX,lambdaAF,AAFin,app.a.tauD,app.a.DD,app.a.thetaTc,app.a.tauDL,app.p.thetaU,app.a.tauDU,tauL,tauLD,tauLU,QdotM,QdotSt,CL,t,y);

    % if all(app.a.inoculum == 0) || any(app.a.inoc_occ == 1)
    [~,y] = ode15s(ODE_concbal,[0 app.a.deltat],[app.v.cXL(previdx) app.v.cS1L(previdx) app.v.cS2L(previdx) app.v.cP1L(previdx) app.v.cOL(previdx) app.v.pO2(previdx) app.v.CB1Ltot(previdx) app.v.CB2Ltot(previdx) app.v.CAcLtot(previdx) app.v.CAlLtot(previdx) app.v.CCLtot(previdx) app.v.pH(previdx) app.v.xO2(previdx) app.v.xCO2(previdx) app.v.hF(previdx) app.v.AAF(previdx) app.v.thetaD(previdx) app.v.thetaL(previdx)]);
    ...
    if app.p.f_Inoc == 1 && app.a.inoc_occ == 0
        ...
        app.a.inoc_occ   = 1;
        logMessage(app, "Process Event", "Inoculation occured")
    else
        app.v.cXL(idx)    = y(end,1);
    end
    app.v.cS1L(idx)   = y(end,2);
    app.v.cS2L(idx)   = y(end,3);
    app.v.cP1L(idx) = y(end,4);
    app.v.cOL(idx)    = y(end,5);
    app.v.pO2(idx)    = y(end,6);
    app.v.CB1Ltot(idx) = y(end,7);
    app.v.CB2Ltot(idx) = y(end,8);
    app.v.CAcLtot(idx) = y(end,9);
    app.v.CAlLtot(idx) = y(end,10);
    app.v.CCLtot(idx)  = y(end,11);
    app.v.pH(idx)      = y(end,12);
    app.v.xO2(idx)     = y(end,13);
    app.v.xCO2(idx)    = y(end,14);
    app.v.hF(idx)      = y(end,15);
    app.v.AAF(idx)     = y(end,16);
    app.v.thetaD(idx)  = y(end,17);
    app.v.thetaL(idx)  = y(end,18);
    ...
    app.v.QS1in(idx) = (app.v.FR1(previdx)*app.p.cS1R1)/app.v.VL(previdx);
    ...
    if any(app.a.VolumeFlag == 1) && any(app.a.VLflag == 0)
        ...
        errordlg(msg);hat die
        logMessage(app.Startingscreen, app.projectID, msg)
        %  app.a.PauseButton.Text = "Resume";
    end

    % Safe new index
    app.nxtidx = idx;

    % Upload new app values
    app_new = app;
end